...
public class PinchableScrollRect:ScrollRect {
    ...
    [SerializeField] private float _zoomLerpSpeed = 8f;          // Increased for snappier feel
    ...
    private float _startPinchDist = 0f;
    ...
    protected override void Awake() {
        base.Awake();                          // FIX: was missing — ScrollRect needs its own Awake
    ...
    // Prevent panning while a pinch gesture (or its immediate aftermath) is active
    ...
    private void HandleTouchInput() {
        if (Input.touchCount == 2) {
            ...
            // FIX: also unblock pan when only 1 finger remains (was only unblocking at 0)
    ...
    // Smoothly lerp the content's local scale toward the target zoom
    ...
    private void OnPinchStart() {
        ...
        // FIX: removed erroneous "* _startPinchScreenPosition.x" — distance is purely
        //      spatial and must not scale with screen coordinate values
    ...
    private void OnPinch() {
        ...
        // FIX: removed erroneous "* centerOfTouch.x" — same bug as in OnPinchStart
        float currentPinchDist = PinchDistanceInContentSpace(pos1,pos2);

        // Guard against a zero start distance to avoid divide-by-zero
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Shared utility: reposition the content pivot under a screen point so
    // that zooming expands/contracts around that point.
    // Extracted to remove the duplicated block that existed in both the
    // mouse-wheel handler and OnPinchStart.
    // ─────────────────────────────────────────────────────────────────────

    private void SetZoomPivotFromScreenPoint(Vector2 screenPoint) {
        // Convert the screen point to local content space
    ...
    // Returns the distance between two touch points measured in content-local space
    ...
    // Adjusts the pivot without visually moving the RectTransform
    private static void SetPivot(RectTransform rectTransform,Vector2 pivot) {
        ...
        // FIX: was incorrectly using Vector3 constructor for a 2D operation;
        //      also scale is already applied by ScrollRect — multiply only by localScale.x
