...
public class UIManager:MonoBehaviour {
    ...
    [SerializeField] private GameObject _startPuzzleButton;
    [SerializeField] private GameObject _timer;

    [SerializeField] private Button[] _footer01Buttons;
    ...
    public void Initialize(GameManager gameManager) {
        ...
        SetFooterButtonInteraction(false);
        SetStartButton(true);
    }

    private void SetFooterButtonInteraction(bool interactable) {
        foreach (Button button in _footer01Buttons) {
            if (button != null)
                button.interactable = interactable;
        }
    }
    private void SetStartButton(bool isEnable) {
        _startPuzzleButton.SetActive(isEnable);
        _timer.SetActive(!isEnable);
    }
    private void SetTimer(bool isEnable) {
        _timer.SetActive(isEnable);
        _startPuzzleButton.SetActive(!isEnable);
    }
    // ─────────────────────────────────────────────────────────────────
    ...
    public void ExitLaunchMode() {
        ...
        SetFooterButtonInteraction(true);
        SetTimer(true);
    }
    ...
    public void MatchPreviewToBoard(RectTransform boardPanel) {
        if (_previewPanelObject == null || boardPanel == null)
            return;
    ...
    private void ValidateReferences() {
        if (_titleLabelText == null)
            Debug.LogError("UIManager: Header/Title is missing.");
        if (_statusMessageText == null)
            Debug.LogError("UIManager: Header/Status is missing.");
        if (_timerLabelText == null)
            Debug.LogError("UIManager: Footer/Timer is missing.");
        if (_bestTimeLabelText == null)
            Debug.LogError("UIManager: Footer/Best is missing.");
        if (_resetPuzzleButton == null)
            Debug.LogError("UIManager: Footer/ResetButton is missing.");
        if (_previewToggleButton == null)
            Debug.LogError("UIManager: Footer/PreviewButton is missing.");
        if (_newImageButton == null)
            Debug.LogError("UIManager: Footer/NewImageButton is missing.");
        if (_previewPanelObject == null)
            Debug.LogError("UIManager: PreviewPanel is missing.");
        if (_previewImage == null)
            Debug.LogError("UIManager: PreviewPanel/PreviewImage is missing.");
        if (_previewImageAspectRatioFitter == null)
            Debug.LogError("UIManager: PreviewPanel/PreviewImage AspectRatioFitter is missing.");
        if (_discoverArtButton == null)
            Debug.LogWarning("UIManager: _discoverArtButton is missing.");
        if (_browseScreen == null)
            Debug.LogWarning("UIManager: _browseScreen is missing.");
    }
